...
import { Play, Search } from "lucide-react"; // ✅ Imported Search Icon
...
function StoneCollection() {
  ...
  // ✅ Added gemname to INITIAL_FILTERS for the search bar
  ...
  const handleProductClick = (productSlug, e) => {
    ...
    window.open(`/gemstones/${productSlug}`, "_blank", "noopener,noreferrer");
  };
  ...
  return (
      <div className="flex flex-col px-3 sm:px-6 md:px-10 py-6 w-full">
        ...
        <div className="w-full text-center mt-2 mb-4 sm:mb-6">
          ...
          {/*      className="w-full border border-gray-300 rounded-full px-4 py-2.5 pl-11 text-sm focus:outline-none focus:border-[#264A3F] focus:ring-1 focus:ring-[#264A3F] transition-shadow shadow-sm"*/}
          {/*      value={selectedFilters.gemname || ""}*/}
          {/*      onChange={(e) => {*/}
          {/*        setSelectedFilters(prev => ({ ...prev, gemname: e.target.value }));*/}
          {/*        setCurrentPage(1);*/}
          {/*      }}*/}
          {/*  />*/}
        ...
        <div className={`w-full grid ${products.length === 1 ? "place-items-center grid-cols-2" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"} gap-4 sm:gap-6 mt-4 sm:mt-6`}>
          {products.map((product) => (
              <div
                  key={product._id}
                  className="rounded-[16px] shadow-sm hover:shadow-xl transition-all duration-300 bg-white flex flex-col items-center pt-6 sm:pt-8 relative cursor-pointer pb-6 border border-gray-100 group"
              ...
              >
                ...
                {/* ✅ FIXED IMAGE SIZING: Uses w-full and responsive heights so it doesn't look tiny in big cards */}
